...
export default function LoginComp() {
    ...
    return(
        ...
        <Formik
          ...
          onSubmit={(values, { setSubmitting }) => {
            setTimeout(() => {
                ...
                let emailFound = data.find((u)=> u.email.toLowerCase()===values.email.toLowerCase());
                let passwordFound = data.find((u)=> u.password.toLowerCase()===values.password.toLowerCase())
                if (!emailFound){
        ...
        <p className=" text-center mt-[20px] ml-[10px] mr-[10px]">Don't have an account? <Link to="/signup">Signup</Link></p>
      </div>
    )
}