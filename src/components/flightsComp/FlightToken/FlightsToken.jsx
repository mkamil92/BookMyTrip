...
import flightPayment from "../../../context/FlightsPaymentContext/FlightPayContext";
import Buttonss from "../../busesComp/button";

export default function GeneratedToken({toknstyle}) {
    const navigate = useNavigate();
    const { toPDF, targetRef } = usePDF({filename: 'token.pdf'});
    const {flightpaymentData} = useContext(flightPayment);
        ...
        return(
            <section id="token" className={`w-full transition ${toknstyle}`}>
                ...
                <Card ref={targetRef}  className=' mx-auto  w-[600px] mr-[30px] max-[647px]:w-[90%] max-[800px]:mr-0'>
                        <div className='bg-[#0000ff4f] text-white pl-[22px] pt-2 pb-2 text-[12px]'>{flightPayment.date}</div>
                            <div className=' w-full flex justify-between pl-[20px] pr-[20px] max-[800px]:flex-col'>
                            <div className=' mt-[35px] mb-[35px] mr-[20px]'>
                                <div>
                                <img src={`/images/flightsImages/${flightpaymentData.busService.airline}.png`} className='w-[100px] h-[100px]'/>
                                <p className=' mt-[10px] tracking-widest'>{flightpaymentData.busService.airline}</p>
                                </div>
                                <Card.Title>{flightpaymentData.busService.time}</Card.Title>
                                <Card.Subtitle className="mb-2 text-muted mt-3">{flightpaymentData.travelFrom}&nbsp;&nbsp;&nbsp;--&nbsp;&nbsp;&nbsp;{flightpaymentData.travelTo}</Card.Subtitle>  
                            </div>
                            <div className=' border-l max-[800px]:border-none'>
                                <div className=' mt-[20px] max-[800px]:mt-0'>
                                    ...
                                    <div className='flex justify-center flex-wrap w-[150px]'>
                                        {
                                            flightpaymentData.seats.map((seat)=>{
                                ...
                                <p className='text-[19px] mt-[-1px] text-[#f5b342]'>{flightpaymentData.totalCost}</p>
                                </div>
                                </div>
                            </div>
                            </div>
