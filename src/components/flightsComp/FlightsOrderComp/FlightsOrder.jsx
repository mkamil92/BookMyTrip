...
import BillingFormsFlights from '../flightBilling/BillingForm';

export default function FlightsOrders() {
    const [billfotmStyle, setBillFormStyle] = useState("scale-[0] absolute")
    const { setFlightpaymentData } = useContext(flightPayment)
    let data = localStorage.getItem("BookingCart");
    let tripData = JSON.parse(data);

    const paymentHandler = () => {
        setFlightpaymentData(tripData)
        setBillFormStyle("scale-[100%] relative")
    }

    if (tripData) {
        return (
            <section className=' w-full pt-[30px] pb-[30px]'>
                    ...
                    <Card className=' w-[100%] mr-[30px] max-[800px]:mr-0'>
                        ...
                        <div className=' w-full flex justify-between pl-[20px] pr-[20px] max-[800px]:flex-col'>
                            <div className=' mt-[35px] mb-[35px] mr-[20px]'>
                                <div>
                                    <img src={`/images/flightsImages/${tripData.busService.airline}.png`} className='w-[150px] h-[150px]' />
                                    <p className=' mt-[10px] tracking-widest'>{tripData.busService.airline}</p>
                                </div>
                            ...
                            <div className=' border-l max-[800px]:border-none'>
                                ...
                                <div>
                                    ...
                                    <div className=' flex justify-end'>
                                        ...
                                        <p className='text-[19px] mt-[-1px] text-[#f5b342]'>{tripData.busService.price}</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                ...
                <BillingFormsFlights billStyle={billfotmStyle} />
            </section>
        )
    } else {
        return (
